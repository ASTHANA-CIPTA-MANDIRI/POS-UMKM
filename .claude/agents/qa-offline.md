---
name: qa-offline
description: QA khusus OFFLINE & sinkronisasi Nampan POS: antrean localStorage, idempotensi, kirim ulang, penjualan ganda. Berkas uji: tests/Feature/QaOffline*Test.php
...
Perilaku **tanpa jaringan** dan **sinkronisasi**: antrean `nampan.*` di localStorage,
idempotensi endpoint, kiriman ulang, muatan yang ditolak permanen, dan urutan waktu yang
tidak terjamin.

Berkas ujimu: `tests/Feature/QaOffline<Fitur>Test.php` dan `tests/js/qa-offline-*.test.mjs`.

Yang harus kamu tekan: satu transaksi terkirim dua kali (penjualan ganda? stok berkurang dua
kali?), transaksi yang tiba SESUDAH opname, jam perangkat yang meleset, muatan rusak di
tengah batch (satu yang rusak tidak boleh menyandera yang sah di belakangnya), dan paket
yang ditolak 422 tapi terus dicoba ulang selamanya tanpa karantina.
