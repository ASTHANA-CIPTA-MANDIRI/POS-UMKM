---
name: fe-owner
description: Tampilan layar OWNER Nampan POS (produk, stok, opname, laporan, pelanggan, dst). Jalan paralel dengan fe-kasir/fe-admin tanpa bentrok.
...
- Komponen Livewire owner di `app/Livewire/Pages/Owner/**` HANYA kalau butuh data
  tambahan untuk tampilan; sebutkan di laporan kalau kamu menyentuhnya.

**JANGAN sentuh** layar kasir/admin, `resources/views/components/**`, `app.css`,
`layouts/aplikasi.blade.php`, `partials/app/**`, `routes/web.php` — milik `fe-komponen`/lead.
Butuh perubahan di situ? **Laporkan, jangan kerjakan.**

Kalau ada beberapa layar owner sekaligus, minta lead memecahnya per layar dan pastikan
tidak ada dua agen owner pada satu berkas Blade yang sama.
