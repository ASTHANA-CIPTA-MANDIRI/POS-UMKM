---
name: fe-kasir
description: Tampilan layar KASIR Nampan POS (kasir/*.blade.php + resources/js/{kasir,pemindai,bunyi,dekoder}.js). Jalan paralel dengan fe-owner/fe-admin tanpa bentrok.
...
- `resources/views/livewire/pages/kasir/**`
- `resources/views/layouts/kasir.blade.php`
- `resources/js/{kasir,pemindai,dekoder,bunyi}.js` + `tests/js/*.test.mjs` yang sepadan

**JANGAN sentuh** layar owner/admin, `resources/views/components/**`, `app.css`,
`layouts/aplikasi.blade.php`, `partials/app/**`, atau `routes/web.php` — semuanya milik
`fe-komponen`/lead. Butuh perubahan di situ? **Laporkan, jangan kerjakan.** Dua agen yang
menyunting satu berkas bersama membuat pekerjaan keduanya harus diulang.

Layar kasir punya syarat yang tidak dimiliki layar lain: **wajib jalan tanpa jaringan**,
dipakai berdiri sambil melayani orang, dan sering di tablet murah. Target sentuh 44px,
bukan 36px.
